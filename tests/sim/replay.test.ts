...
const FIXTURES = [
  { file: 'kite-full-s2.json', hash: 1686675603, check: (m: Record<string, unknown>) => expect(m.clearSeconds).not.toBeNull() },
  { file: 'naive-wave1-s3.json', hash: 4075949549, check: (m: Record<string, unknown>) => expect(m.wavesCleared).toBe(1) },
  { file: 'idle-wave1-s5.json', hash: 1450117791, check: (m: Record<string, unknown>) => expect(m.deaths).toBe(1) },
]
