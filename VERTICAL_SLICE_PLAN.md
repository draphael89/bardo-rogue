...
**Live evidence (2026-08-29, this window):** `pnpm typecheck` green. `pnpm test` **656 / 52 files**. After the type ramp: the exits strip still reads VEIL / BLADE and the rest of the spine; the death stone is NOT YET REBORN / RETURN; the title after a return is DESCEND AGAIN and `1 DESCENT · UNNAMED · 2 KEPT`. Coming home is unarmed, rack waiting, and the banner names the keep — 2 KEPT / the blade waits — not THE BARDO again. Pause is BETWEEN BREATHS / RISE / GIVE THE DESCENT BACK / SETTINGS — no export line. Live overall loop foundation: **76**. Human sessions: zero.
