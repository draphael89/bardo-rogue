...
- `playtest=baseline|no-heavy|no-dash`: arms a playtest session (see `PLAYTEST.md`). The whole session records itself from tick 0, the condition filters LIVE device input only (`no-heavy` strips the independent heavy verb; `no-dash` drops attack intent mid-roll so the dodge-to-attack cancel never fires), and **F4** downloads the session bundle — a valid encoded replay plus a `playtest` key carrying condition, build, counters, and `metrics.summary()`. `pnpm sim -- --replay bundle.json` replays it unmodified. Bots and replays bypass the filter, and the filtered frames are what is recorded, so a bundle replays exactly as the tester played. Three things are interlocked to keep that true: the pause card's **abandon row is absent** while a session is armed (an abandon changes the world outside the frame stream), and **F2/F3 are locked out** (they would restart or end the session's recording).
