...
const hubCandidate = args.hubCandidate === '1'
...
await page.goto(`${url}/?scenario=${scenario}&seed=${seed}&debug=${debug}&mute=${mute}${save}${bot ? `&bot=${bot}` : ''}${actorCandidate ? '&actorCandidate=1' : ''}${heroCandidate ? '&heroCandidate=1' : ''}${hubCandidate ? '&hubCandidate=1' : ''}`)
