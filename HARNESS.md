...
| `pnpm smoke:viewport -- --url <server>` | Resizes one living page through native, wide, portrait, ultra-wide, letterboxed, and back for both title and game HUD; gates target/UI containment and paused-state preservation (`tools/viewport-smoke.ts`; needs a server). |
