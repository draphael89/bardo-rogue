...
async function safeRead(read: (id: string) => Promise<SaveRead>, profileId: string): Promise<{ raw: string | null; failed: boolean; corrupt: boolean }> {
  try {
    const result = await read(profileId)
    return typeof result === 'object' && result !== null
      ? { raw: null, failed: false, corrupt: true }
      : { raw: result, failed: false, corrupt: false }
  } catch { return { raw: null, failed: true, corrupt: false } }
}
...
export async function loadSave(
  ...
  if (live.corrupt || spare.corrupt || current.kind === 'corrupt' || backup.kind === 'corrupt') {
    return { save: current.save, writable: true, source: 'damaged' }
  }
