...
- **Acceptance:** reload → different first-run offers; 100/100 seeds traverse; CI green on PR; death card shows weapon + boons + true killer; pause silences audio ≤100 ms.
...
- **Acceptance:** existing slice reproducible as data (hashes re-pinned once); 6–7-node runs with choices at ≥3 depths; every seeded route reaches Minos (validated in CI); refresh mid-run resumes at the current node in <2 s; two consecutive same-URL runs differ in most rooms.
...
- **Golden screenshots** for overlays (title, reward, map, shop, summaries, pause) diffed in CI at fixed seeds/ticks — the presentation layer finally gets regression cover.
