...
**Revision 4 — Chosen Sentence execution.** The live run now uses one authored `FIRST_GATE`
descent. Its first physical fork says **CUT** or **COMMIT** and truthfully previews the enemy demand,
promised power, and Minos opener. The choice selects an authored branch phrase, a contract-specific
Cocytus echo and reward family, and Minos's first sentence; the Smith remembers it once on return,
even across a reload, and a later light/confirm edge keeps that meeting separate from a purchase.
Remembrances now bank unique rooms actually cleared rather than rooms merely entered, and resolved
runs cannot keep ticking statuses, attacks, projectiles, or waves. Named rivers no longer wear one
another's dress. Automated gates: **930 tests in 77 files**, typecheck, production build, browser
smoke through both endings, and 100/100 seeds for both policies (kite 80% wins, naive 0%). The
...
| Task 4 — generated route + exits map | **Replaced by the Chosen Sentence experiment.** Every live attempt uses `first-gate`; variation lives in the consequential fork rather than six changing lesson orders. The strip says CUT / COMMIT and names the enemy demand, promised power, and Minos answer before the player crosses the physical door. |
...
| Task 4 — half-recognize | **Done (dress + live fill).** Styx, Cocytus, and Phlegethon are pinned to their own named river dress; other combat and elite rooms may still choose a truthful same-kind variant. Elite door glow matches the seal. |
| Task 5 — node-boundary save | **Done.** Schema 3, `src/sim/checkpoint.ts`. Resume re-enters the node from `boundaryRng`. |
| Task 6 — obols / Remembrances / shop | **Done.** Landing's post-fight reward is Charon's stall (`heal` / `vessel` / `vow`). Obols die with the attempt; `finishRun` banks Remembrances only for unique final-wave clears, plus the existing victory bonus. |
...
2. **Route variety** — **Replaced by the Chosen Sentence experiment.** Live generation now holds one First Gate lesson order and puts variation into the consequential CUT / COMMIT fork. The five former spines remain only as checkpoint compatibility data so an in-progress descent can finish truthfully. Named rivers keep their own dress; other combat and elite rooms may still choose a truthful same-kind variant.
...
4. **Smith bark** — **Done.** The one you left is the first line; the approach cannot spend, and a later light/confirm edge buys at most one offer. He asks for what you kept, not for remembrances. After a first death he still says You came back thinner. The anvil still spends. — the banner no longer whispers it in lowercase. A claimed vow keeps its written case too.
